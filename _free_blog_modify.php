...
if(isset($img1)){
  ...
  if(move_uploaded_file($tmp_name1, $dir.$file_name1)){
    ...
    require ('_conn.php');
    $sql = "UPDATE `free_blog` SET `title`='$title',`content`='$content',`nickname`='$nickname',`update_date`=current_timestamp(),`img_file1`='$file_name1',`img_size1`='$img_size1' WHERE no = $no";
    mysqli_query($conn, $sql);
    header("refresh:0.1 url=free_blog.php");
}
  }else {
    require ('_conn.php');
    $sql = "UPDATE `free_blog` SET `title`='$title',`content`='$content',`nickname`='$nickname',`update_date`=current_timestamp()  WHERE  no = $no";
    mysqli_query($conn, $sql);
    header("refresh:0.1 url=free_blog.php");
  }
}else{
