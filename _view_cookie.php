<?php
// ★ 쿠키로 조회수 증가 처리 : free_post.$no 라는 파일명으로 해당 게시판 폴더 안에 쿠키 생성 (로컬파일)
...
if(!empty($no) && empty($_COOKIE['free_post'.$no])) { // $no 변수는 있는데, 해당쿠키는 없을 경우, 미방문자임
	$viewsql = "UPDATE free_post SET view=view+1 WHERE no = $no"; // 미방문자이므로 view를 +1 증가시킨다.
  ...
  $viewresult = mysqli_query($conn, $viewsql);
	...
	<?php } else { // result변수가 생성된 경우 : 정상적 경우
    // <!-- 쿠키로 조회수 제한 설정 -->
    // 용법 : setcookie('쿠키명, 쿠키값, 쿠키유지시간, 경로')
		// 설명 : free_post.$no 라는 파일명으로 쿠키를 생성한다. 쿠키는 empty와 더불어 쓰는, 일종의 스위치와 같다. 장점은 서버에 무리를 주지 않는다.
    // 쿠키명 : 해당게시판폴더/free_post.$no 의 이름으로 생성될 것이다.
    ...
    // 경로 : ''라면 일시 게시판이 포함된 여는 폴더가 두개라면 두번의 조회가 가능하며, '/'이라면 몇개의 폴더에 겹쳐있더라도 하루 한번의 조회만 가능하다.
		setcookie('free_post'.$no, TRUE, time() + (60 * 60 * 24), '/');
	  }
  }
