...
// <!-- 게시물에 해당하는 코멘트 불러오기 -->
$sql_comment = "SELECT * FROM `post_comment` WHERE del_flg='0' AND cid='free_post' AND no={$v['no']}";
...
<form action="_comment_write.php" method="post">
  <input type='text' name='comment' placeholder='댓글을 달아주세요.(100자)' maxlength="100">
  <input type='hidden' name='no' value='<?=$v['no']?>'>
  <input type='hidden' name='cid' value='free_post'> <!-- ★ 블로그 ID : cid는 blogid와 동일하게 설정한다. cid가 없으면 타게시판/블로그와 no 동일할 경우에 다른 곳의 댓글이 여기서도 댓글이 보이게 된다. -->
