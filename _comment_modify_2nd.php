...
$id_get = "SELECT * FROM `post_comment` WHERE `cno` = $cno";
...
if(isset($_SESSION['id'])){
  ...
  if($_SESSION['id'] == $mod['id']){ // 세션 아이디가 해당 글의 아이디와 동일한 경우 = 성공
      ...
      // 수정 실행
      $sql = "UPDATE `post_comment` SET `nickname`='$nickname', `comment`='$comment', `update_date`=CURRENT_TIMESTAMP() WHERE `cno`=$cno";
